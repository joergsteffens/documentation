...
* Local Image Backup (fast recovery of student computers, reserved for the public sector (e.g. schools))
* Linux Agent
...
The `/etc/opsi/backendManager/dispatch.conf` file defines the backends and their purpose.

The file `/etc/opsi/backendManager/acl.conf` defines who has access to which methods.

The `/etc/opsi/backendManager/extend.d` defines opsi extended methods. 
