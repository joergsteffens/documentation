...
This option sends a WakeOnLan signal to the selected clients. Starting on version 4.0.7 you can choose:

* if the wake up signal will be sent immediately
* configure wake delays between clients
* schedule wake events via scheduler

If a client resides in a depot server different from the configserver sending the WakeOnLan signal, '{opsi-configed}' will attempt to connect and relay the message to this 'opsiconfd' depot via HTTPS. This second server will in turn deliver the WakeOnLan signal to the client.

.'{opsi-configed}': WakeOnLan scheduler
image::opsi-configed-wake-on-lan-scheduler.png["{opsi-configed}: WakeOnLan scheduler", width="120mm"]

Please note '{opsi-configed}' triggers the WakeOnLan signal and must therefore remain running. If you intend to use this feature, do not shutdown or close '{opsi-configed}'.
