...
Technical precondition is opsi 4.0.5 with following package versions:
...
. The opsi-linux-bootimage fetches the kernel and initrd for the distribution and unpacks it on the temporary partition.
...
The product 'l-opsi-server' serves to install opsi on a Linux computer with the help of the opsi-linux-client-agent
in an automated way. This can serve to install quickly a new opsi-depot-server or e.g. an opsi test system.

CAUTION: Currently for a opsi-config server an other machine can't be a opsi-linux-client and a opsi-depot-server at the same time. +
