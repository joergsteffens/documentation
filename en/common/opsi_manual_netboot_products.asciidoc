...
==== Parameters for the opsi linux bootimage

The opsi-linux-bootimage has a number of settings which influence the behaviour of the bootimage.
If the screen remains black after loading the boot image or the network card does not work (correctly), the start parameters of the bootimage may have to be adjusted for this specific hardware. +
You can do this in '{opsi-configed}' in the tab 'Host parameters' at the entry 'opsi-linux-bootimage.append'.

Typical values are (individually or combined):
...
Another important default setting is the password of the root user within the opsi-linux-bootimage. This password is 'linux123' by default and you should change this for security reasons.

In order to carry out these modifications, it is best to change the 'opsi-linux-bootimage.append' entry in the 'server-configuration'.

The important option here is 'pwh'. This option must be given the encrypted password as a hash value. This is then automatically loaded into the file '/etc/shadow' when booting. This means that the password is changed before a login on the client is possible. There are several ways to get this hash. To make sure that you get the right hash in the right formatting and type (MD5, SHA1, etc ...), we recommend the following procedure.

If you want to start a Python script before the actual netboot product, there are the following parameters:

* +pre-execute+

* +pre-script+

In addition, these parameters require an address from which the corresponding script is to be downloaded. This can be an 'http://' or 'tftp://' address. Here is an example:

* +tftp://172.16.166/linux/test.py+

Please note that the port for communication with the tftp server is set to 69 by default.

Start an opsi-client via PXE or with the current opsi-client boot CD. Then establish a connection as root via putty or directly from the '{opsi-server}' via ssh. From the '{opsi-server}':
...
The password is linux123. Then simply reset the password of root:
...
Now you have to get the set hash.
...
To set the password, it is sufficient to copy the hash that begins after the first colon and ends with the second colon in the line. The resulting option for the configuration 'opsi-linux-bootimage.append' would be:
...
==== Automatic unattended automated OS installation
...
* Using PXE Boot:
** Via '{opsi-configed}' or '{opsi-admin}' the PC for the new installation is selected.
**  During the next boot process, the client detects with the help of the PXE boot prom, that it should be reinstalled and loads a boot image from the '{opsi-server}'.

* Using CD Boot:
** The client boots the boot image from the opsi-client boot CD.
** The boot image asks for the host name of the client and the opsi server service url.

* Whichever boot option is chosen, the re-installation will procede as follows:
** The boot image starts and asks whether the PC should actually be reinstalled. After confirming this, the installation proceeds without any further request for interaction.
** The boot image partitions and formats the hard drive.
** The boot image transfers the necessary installation files and configuration information from the '{opsi-server}' to the client and initiates a reboot.
** After the reboot, the client automatically installs the operating system based on the transferred configuration information.
** Then the opsi-client-agent is installed to integrate the automated software distribution.
** The automated software distribution then installs all software packages as defined in the configuration of this client.
