...
. Das opsi-linux-bootimage holt sich das distributionseigenen Kernel und Initrd und entpackt es auf der Hilfspartition.
...
===== Die Produkte: debian9, debian10 und ubuntu16-04, ubuntu18-04 und ubuntu20-04
...
* `installation_method`: +
Bietet eine Möglichkeit den Start des Installers zu steuern. Entweder über das übliche kexec oder über einen lokal installierten Installer mit Antwortdatei.
Dies ist aktuell nur mit den Produkten `debian10`, `ubuntu18-04` und `ubuntu20-04` im UEFI Modus möglich.
...
Das Produkt hat über die oben genannten Properties eines z.B debian9 Produktes noch die folgenden zusätzlichen UCS spezifischen Properties:
...
URL=https://download.uib.de/opsi4.1/stable/packages/linux/netboot/
FILE=debian8.tgz
...
#! /bin/bash
DOCUMENTROOT=/var/www/html
URL=https://download.uib.de/opsi4.1/stable/packages/linux/netboot/univention-repository/
FILE=univention-repository-4.4.tgz
mkdir -p ${DOCUMENTROOT}/opsi
cd ${DOCUMENTROOT}/opsi
wget ${URL}/${FILE}
tar xzf ${FILE}
...
CAUTION: Ein selbst erstelltes Repo auf Basis einer UCS 4.4-X DVD führt zu einem uvollständigem Repository. Hierbei ist das Paket `debootstrap` nicht fähig ein UCS 4.2-0 zu installieren. Das von uns bereitgestellte Repository ist hiervon nicht betroffen.
...
ISO-File kopieren nach `/var/lib/opsi/depot/opsi_nfs_share/sles12sp1/64/`
...
Das Produkt ist UEFI/GPT kompatibel (getestet für release=xenial, bionic und focal).
...
Das Produkt ist UEFI/GPT kompatibel (getestet für release=stretch und buster).
...
(Default="https://download.opensuse.org/repositories/home:/uibmz:/opsi:/4.1:/") +
...
Die Meisten der opsi v4.1 Linux Netbootprodukte sind UEFI/GPT kompatibel. Details siehe hierzu in der obigen Auflistung der Netbootprodukte.
